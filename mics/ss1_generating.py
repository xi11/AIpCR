...
cws_folder = os.path.normpath(r'Z:\TIER2\artemis_lei\discovery\til\1_cws_tiling')
annotated_dir = r'Z:\TIER2\artemis_lei\discovery\til\1_cws_tiling'
output_dir = r"Z:\TIER2\artemis_lei\discovery\til\1_cws_tiling"

annotated_files = sorted(glob(os.path.join(annotated_dir, '540*.svs')))
...
for i in range(0, len(annotated_files)):
    ...
    if os.path.exists(os.path.join(output_dir, wsi_name, "Ss1.jpg")):
        ...
        slide_dimension_ss1 = (slide_dimension / 16).astype(np.int)
        ...
        print('\n%s, Target size: %i,%i, level_2 size: %i,%i' % (os.path.basename(filename), w, h, slide_dimension_ss1[0], slide_dimension_ss1[1]))
        img_all = np.zeros((max(h, slide_dimension_ss1[1]), max(slide_dimension_ss1[0],w), 3))
        ...
        for ii in images:
            ...
            # if int(cws_divisor_h) != 1 or int(cws_divisor_w) != 1:
            #    print("\ncws dimension doesn't match with the actual value, resizing to %i by %i"%(cws_h, cws_w))
            # int?
            #    img = cv2.resize(img, (int(img.shape[1] * cws_divisor_h), int(img.shape[0] * cws_divisor_w)), interpolation=cv2.INTER_MAX)

            # img = cv2.resize(img, (int(img.shape[1]*d_y), int(img.shape[0]*d_x)))
            ...
            if i_count % 20 == 0 or i_count >= len(images):
                # print("save", w_i)
                cv2.imwrite(os.path.join(output_dir, imagename, "Ss1.jpg"), img_all)
