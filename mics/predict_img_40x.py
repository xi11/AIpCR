        ...
        for h in range(int(math.ceil((img_h - img_patch_h) / stride_h + 1))):
            for w in range(int(math.ceil((img_w - img_patch_w) / stride_w + 1))):
                ...
                if self.label_diff_pad_h == 0 and self.label_diff_pad_w == 0:
                    image[start_h:end_h, start_w:end_w, :] +=cv2.resize(patches[iter_tot, :, :,:], (1536, 1536))
...
#model.summary()
save_dir = r'T:\project\bcss\patch1536\TMEseg_lungUpdateNorm1batch'
if not os.path.exists(save_dir):
    os.makedirs(save_dir)
    #os.makedirs(os.path.join(save_dir, 'mask_cws'))

datapath = r'T:\project\bcss\patch1536\image'
...
for im_f in imgs:
    ...
    if not os.path.exists(os.path.join(save_dir, img_name + '.png')):
        ...
        testImgc = cv2.resize(testImgc, (384, 384))
        ...
        merge_output = np.squeeze(outData, axis=0)
        ...
        seg_mask = get_colored_segmentation_image(merge_output, 6, colors=class_colors5)
