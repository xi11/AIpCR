...
def generate_tme(datapath, save_dir, file_pattern='*.svs', nfile=0, patch_size=384, patch_stride=192, nClass=2, color_norm=True):
    model = load_model('artemis_tme_sum12_e50_sCE_img768_penmark636.h5', custom_objects={'tf': tf}, compile=False)
    ...
    for im_f in imgs:
        ...
        if not os.path.exists(os.path.join(save_dir_file, img_name + '.png')):
            ...
            seg_mask = get_colored_segmentation_image(merge_output, nClass, colors=class_colors6)
