...
def cell_count(cell_csv_path, output_path, tmeSS1_path, cutoff, file_name=None, classes=('f', 'l', 't', 'o')):
    if file_name is None:
        file_name = os.path.basename(cell_csv_path)

    cellPos = pd.read_csv(cell_csv_path)
    cellPos = cellPos[cellPos['Area_contour'] >= cutoff]
    ...
    yellow_color = np.array([0, 204, 255])  # BGR
    brown_color = np.array([0, 0, 128])  # BGR
    #red_color = np.array([0, 0, 255])  # BGR
    mask_stroma = np.all(tmeSS1 == yellow_color, axis=-1)
    ...
    stroma_oth = cellPos[(cellPos['class2'] == 'o') & (mask_stroma[y_indices, x_indices] == 1)]
    ...
    tumor_oth = cellPos[(cellPos['class2'] == 'o') & (mask_tumor[y_indices, x_indices] == 1)]

    column_names = ['FileName'] + ['#' + c for c in classes] + ['%' + c for c in classes] + ['#f_stroma'] + ['#l_stroma'] + ['#o_stroma'] + ['#l_tumor'] + ['#t_tumor'] + ['#o_tumor']
    row = [file_name] + cell_counts + cell_percentages + [len(stroma_fibro)] + [len(stroma_lym)] + [len(stroma_oth)]+ [len(tumor_lym)] + [len(tumor_can) ]+ [len(tumor_oth)]
...
if __name__ == '__main__':
    ...
    cell_pos_path = '/rsrch6/home/trans_mol_path/yuan_lab/TIER2/artemis_lei/discovery/til/4_cell_class_segformerBRCAartemis/CellPos_area'
    cell_score_path = f'/rsrch6/home/trans_mol_path/yuan_lab/TIER2/artemis_lei/discovery/til/4_cell_class_segformerBRCAartemis/CellScoreOth_area{cutoff}'
    tme_seg_path = '/rsrch6/home/trans_mol_path/yuan_lab/TIER2/artemis_lei/discovery/mit-b3-finetunedBRCA-Artemis-e60-lr00001-s512-20x512/mask_ss1512_orng'
    ...
    for file in files:
        ...
        if not os.path.exists(fileScore_csv):
            cell_count(filePos_csv, fileScore_csv, tme_mask, cutoff, file_name)