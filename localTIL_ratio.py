...
def calculate_lymphocyte_density(image_path, csv_path, patch_save_path, patch_size=256, stride=128):
    ...
    def lymphocyte_density_in_stroma(patch, patch_df, patch_x, patch_y):
        ...
        fibroblast_in_stroma = 0
        for _, fibro in fibroblast_in_patch.iterrows():
            if stroma_mask[fibro['y'] - patch_y, fibro['x'] - patch_x]:
                fibroblast_in_stroma += 1

        return lymphocytes_in_stroma / (lymphocytes_in_stroma + fibroblast_in_stroma + 10e-6)
...
results = []
patch_size = 128 #The diameter of a core is around 1 mm
stride = 128

output_csv = f'/rsrch6/home/trans_mol_path/yuan_lab/TIER2/artemis_lei/discovery/til/localTIL/localtilratio_{patch_size}_str{stride}pad.csv'
