...
def write_to_patch(patch_size, data_files, label_path, save_path):
    for file_n in range(0, len(data_files)):
        ...
        num_patch = extract_patches_img_label(data, labels, save_path, file_base_name, img_patch_h=patch_size, img_patch_w=patch_size, stride_h=patch_size, stride_w=patch_size, label_patch_h=patch_size, label_patch_w=patch_size)
        #print(num_patch)
...
def run(opts_in):
    ...
    files = sorted(glob(os.path.join(data_path, '*.svs')))
    for file in files:
        folder_name = os.path.basename(file)
        train_data = sorted(glob(os.path.join(file, 'Da*')))
        label_path_sub = os.path.join(label_path, folder_name)
        #print('data number: ', len(train_data))
        write_to_patch(patch_size=512, data_files=train_data, label_path=label_path_sub, save_path=save_path)



if __name__ == '__main__':
    opts = {
        'save_path': pathlib.Path('/Users/xiaoxipan/Documents/project/public_data/bcss/patch768'),
        'data_path': pathlib.Path('/Users/xiaoxipan/Documents/project/public_data/bcss/rgbs_colorNormalized'),  #main_file_path -> img_path
        'label_path': pathlib.Path('/Users/xiaoxipan/Documents/project/public_data/bcss/mask_digital'),
