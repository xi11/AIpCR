...
def write_to_patch(patch_w, data_files, label_path, save_path):
    ...
    file_base_name = os.path.basename(data_files)[:-4]
...
def extract_patches_img_label(image, label, save_path, file_base_name, img_patch_h=384, img_patch_w=384, stride_h=384, stride_w=384, label_patch_h=384, label_patch_w=384):
    if image.shape[0] < img_patch_h*0.75 or image.shape[1] < img_patch_w*0.75:
        ...
        for h in range(int(math.ceil((img_h - img_patch_h) / stride_h)+1)): #+1 means consider the edge
            for w in range(int(math.ceil((img_w - img_patch_w) / stride_w)+1)): #+1 means consider the edge
                start_h = h * stride_h
...
def run(opts_in):
    ...
    files = sorted(glob(os.path.join(data_path, '*.png')))
    for file in files:
        file_name = os.path.basename(file)
        train_data = file
        label_path_sub = os.path.join(label_path, 'mask_'+file_name)
        write_to_patch(512, data_files=train_data, label_path=label_path_sub, save_path=save_path)



if __name__ == '__main__':
    opts = {
        'save_path': pathlib.Path('/Users/xiaoxipan/Documents/project/public_data/wsss4luad/size256abovex10/patch512_cartilage'),
        'data_path': pathlib.Path('/Users/xiaoxipan/Documents/project/public_data/wsss4luad/size256abovex10/patch1024_cartilage/image'),  #main_file_path -> img_path
        'label_path': pathlib.Path('/Users/xiaoxipan/Documents/project/public_data/wsss4luad/size256abovex10/patch1024_cartilage/maskPng'),
